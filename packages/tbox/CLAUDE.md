# CLAUDE.md (packages/tbox)

`packages/tbox` (pesde 이름 `qwreey/tbox`) 에서 작업하는 에이전트를 위한 안내입니다.
워크스페이스 전체에 적용되는 규칙(코드 스타일, pesde 사용법 등)은 저장소 루트의 `CLAUDE.md` 를 먼저 보세요.
이 문서는 `tbox` 패키지 고유의 내용만 다룹니다.
...
- 그래서 `test/schema/roblox/` 에서는 Roblox 값을 만들 수 없습니다. 새 Roblox 타입을 추가하면 타입 빌드
